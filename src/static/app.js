    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsMarkup = details.participants.length
          ? `<div class="participants"><p><strong>Participants (${details.participants.length}):</strong></p><ul>${details.participants
              .map(
                (participant) =>
                  `<li><span class="participant-email">${participant}</span><button type="button" class="participant-delete" data-activity="${encodeURIComponent(
                    name
                  )}" data-email="${encodeURIComponent(participant)}" aria-label="Remove ${participant}">×</button></li>`
              )
              .join("")}</ul></div>`
          : `<div class="participants"><p class="participant-empty">No participants have signed up yet.</p></div>`;
